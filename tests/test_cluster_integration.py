...
def test_cluster_integration():
    ...
    # data_arrs may be list of tensors with first dim N
    first_col = ds.data_arrs[0]
    assert first_col.shape[0] == metadata['N'], f"data_arrs rows mismatch: {first_col.shape[0]} != {metadata['N']}"

    # If sampler created variable batch_sizes, ensure they sum to N (or equal to number of mode rows)
    if ds.batch_sizes is not None:
        total = sum(ds.batch_sizes)
        # Depending on sampler behavior, total may equal N or number of mode indices; allow <= N and >0
        assert total > 0 and total <= metadata['N'], f"batch sizes sum {total} unexpected for N {metadata['N']}"
